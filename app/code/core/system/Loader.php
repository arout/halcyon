...
class Loader {
	...
	protected $config;
	protected $session;
	# Used for template hints to display the view file in use
	public $view_file = [];
	...
	public function config($className) {
		...
		foreach ($className as $className) {
			...
			if (is_readable($filename)) {
				...
				echo '<div class="alert alert-danger"><h1>Fatal Error</h1>
                            <h4>Could not load core system configuration file: ' . $className . '.php</h4>
                            Please ensure that the file exists and permission to read the file (644)
                            </div>';
				exit;
			}
	...
	public function view($file, $data = NULL) {
		...
		if (is_readable($dir . $file . '.php')) {
			...
			$this->view_file[$file] = $dir . $file . '.php';
		} elseif (is_readable($dir . $file . '.inc')) {
			require_once $dir . $file . '.inc';
			$this->view_file[$file] = $dir . $file . '.inc';
		} elseif (is_readable($dir . $file . '.html')) {
			require_once $dir . $file . '.html';
			$this->view_file[$file] = $dir . $file . '.html';
		} elseif (is_readable($dir . $file . '.htm')) {
			require_once $dir . $file . '.htm';
			$this->view_file[$file] = $dir . $file . '.htm';
		} elseif (is_readable($dir . $file . '.shtml')) {
			require_once $dir . $file . '.shtml';
			$this->view_file[$file] = $dir . $file . '.shtml';
		} else {
			$filename = $dir . $file;
