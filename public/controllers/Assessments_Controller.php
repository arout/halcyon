...
class Assessments_Controller extends Fusion\System\SystemController {
	...
	public function edit() {
		...
		// Open file in editor
		$data['content'] = $this->draft_folder.$this->session()->fetch('assessment_file_name');
	...
	public function save_draft( $company, $filename, $time, $name ) {
		...
		if( $this->model('Assessments')->save_draft( $this->company, $this->file_name, time(), $this->session()->fetch( 'first_name' ).' '.$this->session()->fetch( 'last_name' ) ) )
			return TRUE;
		else
	...
	public function process_wizard_form_one() {
		...
		if ( $_POST ) {
			...
			$form = $this->toolbox('Sanitize')->form( $_POST );
				...
				$section->addTextBreak(1);
				$section->addText('We took a moment to assess the website for '. $this->session()->fetch('assessment_company').' and these are the observations and 
				...
				$this->model('Assessments')->network_speed( $this->toolbox('Performance')->check_latency_raw( $this->session()->fetch( 'assessment_url' ) ), 
				$this->toolbox('Performance')->check_download_speed_raw( $this->session()->fetch( 'assessment_url' ) ), $this->company );
	...
	public function process_wizard_form_two() {
		...
		$html->load_file( $this->session()->fetch( 'assessment_url' ) );
		...
		$this->model('Assessments')->scripts( $js, $css, $this->session()->fetch( 'assessment_company') );
		...
		$tags = get_meta_tags( $this->session()->fetch( 'assessment_url' ) );
